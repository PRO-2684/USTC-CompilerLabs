...
void translate_main(CodeGen *codegen) {
    ...
    // - 数组的大小是多少字节? (40)
    // - 数组的偏移量是多少字节? (-64)
    offset_map["%op0"] = -24;  // DONE: 请填空
    offset_map["array"] = -64; // DONE: 请填空
    codegen->append_inst("addi.d", {"$t0", "$fp", std::to_string(offset_map["array"])});
    ...
    // 你需要思考 %op1 的偏移量是多少字节
    offset_map["%op1"] = -72; // DONE: 请填空
    // 获得数组的地址, 将其写入 $t0 中
    codegen->append_inst("ld.d", {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    // DONE: 计算 %op1 的值, 并将其写入 %op1 对应的内存空间中
    ...
    // 你可以手动计算 gep 的值, 或者使用指令计算 gep 的值
    // 你需要思考 %op2 的偏移量是多少字节
    offset_map["%op2"] = -80; // DONE: 请填空
    // 获得数组的地址, 将其写入 $t0 中
    codegen->append_inst("ld.d", {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    // DONE: 计算 gep 的值, 并将其写入 %op2 对应的内存空间中
    ...
    offset_map["%op3"] = -84; // DONE: 请填空
    // DONE: 先获得 %op1 的值, 然后获得 %op1 指向的空间的值, 最后将这个值写入
    // %op3 对应的内存空间中
    codegen->append_inst("ld.d", {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    ...
    offset_map["%op4"] = -88; // DONE: 请填空
    // DONE: 先获得 %op3 的值, 然后将其乘以 3, 最后将结果写入 %op4 对应的内存空
    // 间中
    ...
    // DONE: 先获得 %op4 的值, 然后将其写入 %op2 指向的空间中
    ...
    offset_map["%op5"] = -92; // DONE: 请填空
    // DONE: 先获得 %op2 的值, 然后获得 %op2 指向的空间的值, 最后将这个值写入
    // %op5 对应的内存空间中
